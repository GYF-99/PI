      ...
      <div class="content-section">
        ...
        <el-tabs v-model="activeName" @tab-click="handleClick">
          <el-tab-pane label="商品" name="first">
            <ul class="search_ul">
              <li class="adobe-product " v-for="(item,index) in musicList" :key="index" @dblclick="runapi(item.id)">
          ...
          <!-- <el-tab-pane label="专辑" name="foru">
            <div class="apps-card">
              <div class="block" v-for="item in songalbum" @click="opensongalbum(item.id)">
                <div class="imgkd">
                  <el-image class="albumcover app-card" :src="item.picUrl"></el-image>
                </div>
                <h5 class="songlisttext">{{item.name}}</h5>
              </div>
            </div>
  ...
  export default {
    ...
    methods: {
      ...
      runapi(id) {
        //歌曲不同步
        var _this = this;
        this.$axios.all([_this.playMusic(id), _this.getlr(id), _this.getdta(id), _this.getcm(id)]).then(_this.$axios.spread(function (res1, res2, res3, res4) {
          ...
          // console.log("第四个" + res4);
          _this.hotComments = res4.data.hotComments;
